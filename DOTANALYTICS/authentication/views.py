    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if user is not None:
                ...
                response_data = {
                "token": token.key,
                "redirect_url": f'http://localhost:5173/LoginRedirectHandler?token={token.key}&email={user.email}&first_name={user.first_name}&last_name={user.last_name}'
                }

                return redirect("http://localhost:5173/login-redirect")
            else:
                return Response({"error": "Invalid email or password"}, status=status.HTTP_400_BAD_REQUEST)
